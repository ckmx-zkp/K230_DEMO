...
> 当前 PCBA 版本：01Studio CanMV v1.8、CSI0 OV5647、UART2 GPIO44/45；IDE_PREVIEW 控制虚拟屏，HEADLESS 控制无物理屏模式。已新增目标框关联、中心偏移、106 点可视化和五类个人标定表情原型。新增模型与部署/标定步骤见 [人脸与表情验收](docs/face-expression-bringup.md)。下方亚博基线和旧路线图保留作历史参考，表情准确率及多模型新版本尚待真机验收。
...
- [x] 4. 功能 a：人脸朝向识别（已真机验证：欧拉角 + 方向迟滞分类，共享检测框不重复检测）
- [x] 5. 功能 c：手势识别（已真机验证：九种手势几何分类 + 3 次确认防抖，4 模型共存无 OOM）
- [ ] 6. 功能 b：表情分类（含真机阈值标定）
- [ ] 7. 整合调优 + 协议文档补全
